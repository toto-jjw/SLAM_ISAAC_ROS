...
private:
  // Rotation from ORB-SLAM2 optical frame to ROS ENU base frame.
                       ...
                       const Eigen::Quaternionf& qwb)
  {
    ...
    if (have_last_ && dt > 0.0) {
      // Linear velocity: first-order finite difference
      Eigen::Vector3f v = (twb - last_twb_) / static_cast<float>(dt);

      // Angular velocity in body frame: dq = q_last^{-1} * q_curr
  ...
  void publishStatusOK(size_t tracked_pts) {
    ...
    kv.key   = "tracked_map_points";
    kv.value = std::to_string(tracked_pts);
  ...
  void imageCallback(const sensor_msgs::msg::Image::SharedPtr msg) {
    ...
    // 4) Optical frame → ROS ENU frame
    const Eigen::Matrix3f Ropt2base = R_opt2base();
    Eigen::Matrix3f R_wopt_b = Rwc * Ropt2base.transpose();
    Eigen::Matrix3f R_wros_b = Ropt2base * R_wopt_b;
...
int main(int argc, char** argv) {
  ...
  if (argc < 3) {
    std::cerr << "Usage: ros2 run orbslam2_ros2_mono ros2_mono "
              << "<path_to_ORBvoc.txt> <path_to_settings.yaml>\n";
    return 1;
  }
  const std::string voc_path      = argv[1];
  const std::string settings_path = argv[2];

  ORB_SLAM2::System SLAM(voc_path, settings_path, ORB_SLAM2::System::MONOCULAR, /*viewer=*/true);
